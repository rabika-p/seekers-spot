...
const EditFoundItem = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:8080/api/found-items/${itemId}`, config)
      .then((response) => {
  ...
  const onSubmit: SubmitHandler<IFoundItems> = (data) => {
    const userId = localStorage.getItem("userId");

    const formData = new FormData();
    formData.append("itemName", data.itemName);
    formData.append("category", selectedCategory);
    // formData.append("foundDate", data.date);
    ...
    axios
      .put(`http://localhost:8080/api/found-items/${itemId}`, formData, config)
      .then((res) => {
    ...
    <ProtectedRoute isAuthenticated={!!accessToken}>
      <>
        ...
        <div className="w-full overflow-hidden">
          <TitleSection title="Edit found Item" />
          <Card>
            <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="w-1/2">
                  <input
                    {...register("itemName", {
                      ...
                      pattern: {
                        value: /^[a-zA-Z]+( [a-zA-Z]+)*$/, 
                        message: "The item name field can only contain alphabets",
                      },
                    })}
              ...
              {/* <div className="flex justify-center">
                <label className="text-lg w-1/3">Found Date</label>
                <div className="w-1/2">
                  <input
                    {...register("date", {
                      required: "The found date field is required",
                    })}
                    className={`border rounded-lg w-full py-2 px-3 mb-3 mt-2 text-gray-700 shadow-sm  ${
              ...
              <div className="flex justify-center ">
                <label className="text-lg w-1/3">Found Location</label>
                <div className="w-1/2">
                  <input
                    {...register("location", {
                      required: "The found location field is required",
                    })}
                    className={`border rounded-lg w-full py-2 px-3 mb-3 mt-2 text-gray-700 shadow-sm  ${
